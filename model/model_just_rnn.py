...
def build_net_just_rnn(time_step=128,
              spectrum_hight=256,#代表每列语谱图的高度
              spectrum_width = 1,#语谱图的宽度
              num_classes=10,
...
#     X = add_cnn_layers_updated_v10(X)
#     cnn_feature_size = X.get_shape().as_list()[2] #获取feature_size的值
#     X = tf.reshape(X,[-1,cnn_feature_size*time_step])
#     print "CNN num_of_units:",cnn_feature_size
    
    # for RNN
    X = tf.reshape(x_placeholder, [-1,time_step*spectrum_hight*spectrum_width])
        
    #RNN层
    rnn_output = add_dynamic_rnn_layer(X, rnn_out_size, batch_size,spectrum_hight*spectrum_width, time_step,num_layer=rnn_layers,keep_prob=keep_prob_placeholder)
...
#     png_path = "/media/tsiangleo/0FD7048F0FD7048F/DeepLearning/data/china-celebrity-speech/merged_data/final_yanshi/10/png"
    ...
    #配置输出路径
    old_checkpoints_dir = "output/3rnn_256/ckpt"
    new_checkpoints_dir = "output/3rnn_256/batchsized_ckpt"
    graph_dir = "output/3rnn_256/"
    graph_file_name ="one_batchsize_sid.pb"
      
    g = build_net_just_rnn(time_step, spectrum_hight, spectrum_width, num_classes, rnn_out_size,rnn_layers, batch_size)
...
#     png_path = "/media/tsiangleo/0FD7048F0FD7048F/DeepLearning/data/china-celebrity-speech/merged_data/final_yanshi/10/png"
    ...
    #配置输出路径
    checkpoints_dir = "output/3rnn_256/ckpt"
    graph_dir = "output/3rnn_256/const_graph"
    graph_file_name ="graph.pb"
    tensorboard_dir = "output/3rnn_256/board_"+str(num_classes)
