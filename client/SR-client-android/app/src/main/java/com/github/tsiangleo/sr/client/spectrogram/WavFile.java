...
//	}
	...
	{
		out.printf("File: %s\n", file);
		out.printf("Duration(s):"+(numFrames/(double)sampleRate)+"\n");
		out.printf("Channels: %d, totalSamples: %d\n", numChannels, numFrames);
	...
	public String info(){
		...
		sb.append("File: "+file+"\n");
		sb.append("Duration(s):"+(numFrames/(double)sampleRate)+", ");
		sb.append("Channels: "+numChannels+", totalSamples: "+numFrames+"\n");
		sb.append("Sample Rate:"+sampleRate+", Block Align: "+blockAlign+"\n");
		sb.append("Valid Bits: "+validBits+", Bytes per sample: "+bytesPerSample+"\n");
		return sb.toString();
	}
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
	 ...
	 * @throws IOException
	 */
	public static int[] loadWav(File wavfile) throws Exception {
...
//		wavFile.display();
    ...
    =====================  ===========  ===========  =============
	 *
	 * 将short类型的采样值转为0-1之间的float。为了节省空间，以float方式存放转换后的数据。
	 * 
	 * @param samples 每个采样点的格式为：16-bit PCM，将每个采样点的值转为0-1之间的float。
	 * @return
	 */
	public static float[] shortTofloats(short[] samples){
	...
	public static float[] intTofloats(int[] samples){
		float[] floatSamples = new float[samples.length];
		for (int i = 0; i < samples.length; i++) {
			floatSamples[i] = (float) (samples[i]/32768.0);
		}
